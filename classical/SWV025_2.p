...
tff(formula_2_unnamed_formula, axiom, ?[N_i: $int]: (((![X_g: general]: (q(X_g) <=> X_g = f__integer__(N_i)) & $greatereq(N_i, 0)) & $lesseq($product(N_i, N_i), n_i)) & $greater($product($sum(N_i, 1), $sum(N_i, 1)), n_i))).
tff(formula_3_completed_definition_of_q_1, conjecture, ![V1_g: general]: (q(V1_g) <=> ?[X_g: general]: (V1_g = X_g & (?[Z_g: general]: (Z_g = X_g & p(Z_g)) & ?[Z_g: general]: (?[I_i: $int, J_i: $int]: ((Z_g = f__integer__($sum(I_i, J_i)) & f__integer__(I_i) = X_g) & J_i = 1) & (~p(Z_g))))))).
