...
tff(formula_1_unnamed_formula, axiom, ![N_i: $int]: ($product(N_i, N_i) = $product($uminus(N_i), $uminus(N_i)))).
tff(formula_2_completed_definition_of_p_1, conjecture, ![V1_g: general]: (p(V1_g) <=> ?[X_g: general]: (?[I_i: $int, J_i: $int]: ((V1_g = f__integer__($product(I_i, J_i)) & f__integer__(I_i) = X_g) & f__integer__(J_i) = X_g) & ?[Z_g: general, Z1_g: general]: ((Z_g = X_g & ?[I_i: $int, J_i: $int, K_i: $int]: (((I_i = 0 & J_i = n_i) & Z1_g = f__integer__(K_i)) & $lesseq(I_i, K_i) & $lesseq(K_i, J_i))) & Z_g = Z1_g)))).
