...
tff(constraint_0, axiom, ![X: general, Z1: general, Z2: general]: (?[Z: general, Z2: general]: (Z = X & Z2 = Z1 & color(Z, Z2)) & ?[Z: general, Z1: general]: (Z = X & Z1 = Z2 & color(Z, Z1)) & ?[Z: general, Z3: general]: (Z = Z1 & Z3 = Z2 & Z != Z3) => $false)).
tff(constraint_1, axiom, ![X: general]: (?[Z: general]: (Z = X & vertex(Z)) & ?[Z: general]: (Z = X & ~aux(Z)) => $false)).
tff(constraint_2, axiom, ![X: general, Y: general, Z: general]: (?[Z: general, Z1: general]: (Z = X & Z1 = Y & edge(Z, Z1)) & ?[Z1: general, Z2: general]: (Z1 = X & Z2 = Z & color(Z1, Z2)) & ?[Z1: general, Z2: general]: (Z1 = Y & Z2 = Z & color(Z1, Z2)) => $false)).
tff(completed_definition_of_color_2, axiom, ![V1: general, V2: general]: (color(V1, V2) <=> ?[X: general, Z: general]: (V1 = X & V2 = Z & (?[Z: general]: (Z = X & vertex(Z)) & ?[Z1: general]: (Z1 = Z & color(Z1))) & ~~color(V1, V2)))).
tff(spec, conjecture, ![X: general, Z1: general, Z2: general]: (color(X, Z1) & color(X, Z2) => Z1 = Z2)).
