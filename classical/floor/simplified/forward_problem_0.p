...
tff(completed_definition_of_p_1, axiom, ![V1: general]: (p(V1) <=> ?[K$i: $int]: ($lesseq(0, K$i) & $lesseq(K$i, n$i) & V1 = f__integer__(K$i) & $lesseq($product(K$i, K$i), n$i)))).
tff(spec, axiom, ?[N$i: $int]: (![X: general]: (q(X) <=> X = f__integer__(N$i)) & $greatereq(N$i, 0) & $lesseq($product(N$i, N$i), n$i) & $greater($product($sum(N$i, 1), $sum(N$i, 1)), n$i))).
tff(completed_definition_of_q_1, conjecture, ![V1: general]: (q(V1) <=> ?[I$i: $int]: (p(V1) & f__integer__(I$i) = V1 & ~p(f__integer__($sum(I$i, 1)))))).
