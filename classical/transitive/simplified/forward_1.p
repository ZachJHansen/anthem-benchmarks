...
tff(left_0, axiom, ![X: general, Y: general]: (hp(X) & hp(Y) & ~~tq(X, Y) => hq(X, Y) & (tp(X) & tp(Y) & ~~tq(X, Y) => tq(X, Y)))).
tff(left_1, axiom, ![X: general, Y: general, Z: general]: (hq(X, Y) & hq(Y, Z) & hp(X) & hp(Y) & hp(Z) => hq(X, Z) & (tq(X, Y) & tq(Y, Z) & tp(X) & tp(Y) & tp(Z) => tq(X, Z)))).
tff(right_1, conjecture, ![X: general, Y: general, Z: general]: (hq(X, Y) & hq(Y, Z) & ~tq(X, Z) & hp(X) & hp(Y) & hp(Z) => $false & (tq(X, Y) & tq(Y, Z) & ~tq(X, Z) & tp(X) & tp(Y) & tp(Z) => $false))).
