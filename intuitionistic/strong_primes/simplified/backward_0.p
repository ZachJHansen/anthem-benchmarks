...
tff(right_0, axiom, ![I2$i: $int, J2$i: $int]: ($greater(I2$i, 2) & $greater(J2$i, 2) & I2$i = $sum(I2$i, 0) & J2$i = $sum(J2$i, 0) => composite(f__integer__($product(I2$i, J2$i))))).
tff(right_1, axiom, ![I: general]: (?[I1$i: $int]: (I1$i = $sum(I1$i, 0) & f__integer__(I1$i) = I & ~composite(I)) => prime(I))).
